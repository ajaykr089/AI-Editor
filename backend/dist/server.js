...
app.post('/api/files/create', async (req, res) => {
    try {
        ...
        if (!filePath || !type)
            throw new Error('filePath and type required');
        const absolute = resolveSafe(filePath);
        if (type === 'folder') {
...
app.post('/api/files/rename', async (req, res) => {
    try {
        ...
        if (!filePath || !newPath)
            throw new Error('filePath and newPath required');
...
app.post('/api/autocomplete', async (req, res) => {
    try {
        const { content, language, cursorLine, cursorColumn } = req.body;
...
app.post('/api/search', async (req, res) => {
    try {
        ...
        if (!query || query.length < 2)
            throw new Error('query required (min 2 chars)');
        await ensureWorkspace();
